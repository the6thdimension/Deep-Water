...
- **2026-09-08** — Terminal-guidance overshoot root-caused and fixed; a tuning trap catalogued on the way. Deterministic pure-core repro (no play mode: build the L4 stack from the asset, step 0.02 s, log lock/off-boresight/min-distance) found TWO stacked physical defects, neither of them "guidance": (1) authored ClAlpha was body-alone slender-body theory — the 50 g-rated ESSM could physically pull ~9 g at Mach 1.5, so PN saturated; fixed with effective whole-airframe (body+strake+tail) normal-force values on the profile. (2) The cone seeker dropped lock instantly at the terminal LOS swing and went blind for the endgame; fixed with SO-driven track-memory coast (`seekerCoastTimeS`, dead-reckoned observation, seamless reacquire; 0 = legacy). The trap (user caught it): the repro scenario — a below-horizon stationary ground target — is OUTSIDE the SAM's design envelope, and sweeping navigation gain to "fix" it (N=5.5 → 2.6 m!) destabilized the real air-intercept cases (crossFast3000: 13 m → 216 m). Rule candidate: **tune weapon data against the weapon's design envelope; out-of-envelope shots get bounded-behavior tests, not gain chased to zero.** Air battery post-fix at N=3.5: head-on/tail-chase inside the 10 m fuze, crossings 12-16 m (blast covers), ground shot bounded ≤30 m (was 44 m + 150 m-late detonation). 6 new EditMode regression tests pin all of it; 48/50 suite passes — the 2 failures pre-date this session (verified by stash-bisect) and are tracked in ROADMAP.
- **2026-09-08** -- Terminal-guidance follow-ups + full multi-weapon range expansion. **Guidance:** added seeker track-memory coast (dead-reckons through the terminal LOS swing), midcourse datalink (fly on truth until the seeker acquires -- models command/TVM guidance), and a new `BlendedPursuitProNav` law that weights pursuit vs ProNav by closing ratio Vc/|v|. The blend is what makes vertical launch work: pure ProNav is degenerate at ~90 deg off-boresight (Vc~0 -> zero command -> ballistic climb; measured 10.3 km apogee, 3.1 km miss), while pursuit-when-off-course produces the pitch-over as pure geometry (no scripted flight program, no mode flags). Post-fix VLS closes to 20 m with a 791 m apogee; the air-intercept battery is unregressed (head-on 6.4 m, tail-chase 5.7 m, crossing 15.2 m, out-of-envelope ground shot 9.3 m). **Range:** grew MissileRange into an eight-sector multi-weapon range (programmatic Unity Terrain with ridge/mesa/basin, ground+aerial+naval targets, VLS pad, mesa gantry, airborne testbed, demolition pad) via an additive `WeaponsRangeSectorBuilder` that is idempotent per sector and also called from the from-scratch builder. Extracted `DetonationEffects` (shared FX+blast mechanism) so emplaced ordnance (`DemolitionCharge`) detonates identically to missiles without dragging a flight sim along. `WaypointMover` reuses Pat5 (sim-on-FixedUpdate, render-interpolated) for drones and the aircraft. 49/50 EditMode tests pass (the 2 failures pre-date this session, tracked). **Environment gotcha (cost ~30 min):** the project root is a junction (C:/Users/SIX/Unity Projects -> F:); a genuine compile error (missing `using GuidedFury.Core.State;` in a new file) made Unity silently keep the last-good DLL loaded while `AssetDatabase.Refresh` reported success and `isCompiling` briefly flipped true -- looked exactly like a frozen compile pipeline. Lesson: when a Unity assembly will not pick up source edits and the MCP console shows no errors, read the compile errors from the Editor's own console (the user could see two CS errors the MCP ReadConsole filter missed) before blaming the file watcher.
