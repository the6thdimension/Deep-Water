...
namespace NWH.WheelController3D
{
    ...
    public class WheelController : MonoBehaviour
    {
        ...
        /// </summary>
        [UnityEngine.Tooltip("    Friction implementation.")]
        ...
        /// <summary>
        /// Should the ground detection layer mask be automatically set up.
        /// This prevents the wheels from detecting the vehicle itself as ground.
        /// </summary>
        [UnityEngine.Tooltip("Should the ground detection layer mask be automatically set up.\r\nThis prevents the wheels from detecting the vehicle itself as ground.")]
        ...
        [UnityEngine.Tooltip("Higher the number, higher the effect of longitudinal friction on lateral friction.\r\n" +
            ...
            "The 'accurate' value is 1 but might not be desirable for arcade games.")]
        [Range(0, 1)]
        public float frictionCircleStrength = 1f;
        ...
        public float frictionCircleShape = 1.5f;
        ...
        /// </summary>
        [UnityEngine.Tooltip("    Only the layers that are selected/ticked will be detected.\r\n    It is important that the vehicle is not one of them as it will self-detect itself as ground.")]
        ...
        /// </summary>
        [UnityEngine.Tooltip("Total number of wheels on the vehicle.")]
        public int vehicleWheelCount;

        /// <summary>
        /// Is the wheel only being updated for visual purposes? E.g. multiplayer client.
        /// </summary>
        [UnityEngine.Tooltip("Is the wheel only being updated for visual purposes? E.g. multiplayer client.")]
        ...
        /// </summary>
        [Range(0, 5)]
        [UnityEngine.Tooltip("Determines depenetration strength.\r\nWhen the wheel goes through the ground either due to high vertical velocity or due to suspension bottoming out,\r\nsuch as after large jumps, a force is applied to return the wheel back to the above-ground position.\r\nToo low value will result in wheels clipping through ground while a too high value will result\r\nin bounce when suspension bottoms out.")]
        ...
        /// </summary>
        [Range(0, 5)]
        [UnityEngine.Tooltip("Dampens the depenetration strength to prevent it from over-reacting and causing a bounce-back.")]
        ...
        [SerializeField] 
        private float _effectiveCamber;
        ...
        private float smoothedMotorTorque;
        private float smoothedBrakeTorque;
        ...
        {
            get { return wheel.worldPosition; }
        }
        ...
        {
            get { return spring.compressionPercent; }
        }
            ...
            if (wheel.NonRotatingVisual != null)
            {
                wheel.nonRotatingPositionOffset =
                    wheel.Visual.transform.InverseTransformDirection(
                        wheel.NonRotatingVisual.transform.position - wheel.Visual.transform.position);
            }
        ...
        public void OnPrePhysicsSubstep(float t, float dt)
        {
            ...
            {
                if (parentRigidbody.isKinematic) return; // Do not update if kinematic and local
        ...
        public void OnPhysicsSubstep(float t, float dt, int i)
        {
            ...
            _effectiveCamber = Vector3.SignedAngle(wheel.up, wheelHit.normal, wheel.forward);
            float camberCoeff = wheel.camberFrictionCurve.Evaluate(Mathf.Abs(_effectiveCamber));

            frictionModel.Step(wheel.motorTorque, wheel.brakeTorque + dragTorque, 
                                forwardFriction.speed, sideFriction.speed,
                                ref wheel.angularVelocity, wheel.load, wheel.maxLoad, dt,
                                wheel.radius, wheel.inertia, activeFrictionPreset.Curve,
                                activeFrictionPreset.BCDE.z,
                                forwardFriction.forceCoefficient * camberCoeff, sideFriction.forceCoefficient * camberCoeff, 
                                forwardFriction.slipCoefficient, sideFriction.slipCoefficient,
                                ref forwardFriction.slip, ref sideFriction.slip,
                                ref forwardFriction.force, ref sideFriction.force,
                                ref powertrainCounterTorque, activeFrictionPreset.peakSlip, 
                                frictionCircleStrength, frictionCircleShape); // This could be a struct at this point.
            ...
            if (hasHit)
            {
                ...
                parentNRigidbody.AddForceAtPosition(_surfaceForceVector, wheelHit.raycastHit.point 
                    + parentNRigidbody.StepPositionDelta, true);
            }
            else
            {
            ...
            // Add squat torque
            float clampedAntiSquat = antiSquat < -1f ? -1f : antiSquat > 1f ? 1f : antiSquat;
            float squatMagnitude = forwardFriction.force * wheel.radius * clampedAntiSquat;
            Vector3 squatTorque = squatMagnitude * wheel.right;
            parentNRigidbody.AddTorque(squatTorque, true);

            _totalForceThisUpdate += _surfaceForceVector / parentNRigidbody.Substeps;

            if (!_hasBeenEnabledThisFrame)
            {
                float chassisTorque = (wheel.angularVelocity - wheel.prevAngularVelocity) * wheel.baseInertia / dt;
                parentNRigidbody.AddTorque(-chassisTorque * wheel.right, true);
            }
        ...
        public void SetDefaults(bool reset = false, bool findWheelVisuals = true)
        {
            ...
            if (wheel.camberFrictionCurve == null || wheel.camberFrictionCurve.keys.Length == 0 || reset)
            {
                wheel.camberFrictionCurve = GenerateDefaultCamberFrictionCurve();
            }
        ...
        private void WheelUpdate()
        {
            ...
            // Calculate position
            Vector3 newPosition = wheel.worldPosition;

            // Calculate rotation
            Quaternion newRotation = wheel.worldRotation;

            // Apply wobble
            if (_damage > 0)
            {
                newRotation *= Quaternion.AngleAxis(_damage * 30f, Vector3.up);
            }

            // Apply visual position and rotation
            wheel.Visual.transform.SetPositionAndRotation(newPosition, newRotation);
        ...
        public void VisualUpdate()
        {
            ...
            wheel.Visual.transform.SetPositionAndRotation(wheel.worldPosition,
                                                          wheel.worldRotation);
        ...
        private float CalculateDepenetrationForce()
        {
            ...
            if (applyForceToOthers && rigidbodyB != null)
            {
        ...
        private AnimationCurve GenerateDefaultCamberFrictionCurve()
        {
            AnimationCurve ac = new AnimationCurve();
            ac.AddKey(0.0f, 1.0f);
            ac.AddKey(50.0f, 0.67f);
            ac.AddKey(90.0f, 0.0f);
            return ac;
        }
