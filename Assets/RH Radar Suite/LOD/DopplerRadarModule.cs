...
{
    /// <summary>
    /// LOD3: Doppler Radar Module
    /// Includes Doppler processing for radial velocity.
    /// Can identify moving vs. stationary targets.
    /// </summary>
    [AddComponentMenu("RH Radar Suite/LOD Modules/LOD3 Doppler Radar")]
    public class DopplerRadarModule : RadarLODModuleBase
    {
        [Header("Doppler Radar Settings")]
        [Tooltip("Radar beam width in degrees")]
        [Range(1f, 120f)]
        [SerializeField] public float beamWidth = 20f;
        ...
        [Range(1f, 100f)]
        [SerializeField] private float rangeAccuracy = 5f;
        
        [Tooltip("Velocity accuracy in m/s (lower is better)")]
        [Range(0.1f, 10f)]
        [SerializeField] private float velocityAccuracy = 1f;
        
        [Tooltip("Minimum velocity for moving target detection (m/s)")]
        [Range(0.1f, 10f)]
        [SerializeField] private float minDetectableVelocity = 1f;
        
        [Tooltip("Enable clutter rejection (filter out stationary targets)")]
        [SerializeField] private bool enableClutterRejection = false;
        ...
        private void PerformDopplerScan()
        {
            ...
            for (int i = 0; i < targetsFound; i++)
            {
                ...
                if (previousPositions.TryGetValue(target, out Vector3 prevPos))
                {
                    ...
                    radialVelocity = Vector3.Dot(velocity, directionToTarget);
                    
                    // If clutter rejection is enabled and target is not moving fast enough, skip
                    if (enableClutterRejection && Mathf.Abs(radialVelocity) < minDetectableVelocity)
                    {
                        continue;
                    }
                }
                ...
                if (contacts.TryGetValue(target, out RadarContact contact))
                {
                    ...
                    contact.Update(target.transform.position, signalStrength, RadarLOD.LOD3_DopplerRadar, transform.position);

                    // Update velocity information
                    contact.SetVelocity(velocity);
                ...
                else
                {
                    ...
                    contact.Update(detectedPosition, signalStrength, RadarLOD.LOD3_DopplerRadar, transform.position);
        ...
        public override void SetParameter(string paramName, object value)
        {
            ...
            switch (paramName)
            {
                ...
                case "enableFullRotation":
                    if (value is bool enableRotation)
                        enableFullRotation = enableRotation;
                    break;
                case "sectorSize":
                ...
                case "detectionThreshold":
                    ...
                    break;
                case "rangeAccuracy":
                    if (value is float rangeAcc)
                        rangeAccuracy = rangeAcc;
                    break;
                case "velocityAccuracy":
                    if (value is float velAccuracy)
                        velocityAccuracy = velAccuracy;
                    break;
                case "minDetectableVelocity":
                    if (value is float velocity)
                        minDetectableVelocity = velocity;
                    break;
                case "enableClutterRejection":
                    if (value is bool enableClutter)
                        enableClutterRejection = enableClutter;
                    break;
            }
        }
        ...
        public override object GetParameter(string paramName)
        {
            switch (paramName)
            {
                ...
                case "minDetectableVelocity":
                    return minDetectableVelocity;
                case "enableClutterRejection":
                    return enableClutterRejection;
                default:
